...
sub checkManifest
{
	use constant PERM_SIGNATURE => "android.permission.";
	use constant PERMS => (
	...
	my $file = shift;
	if (!open FILE, "./aapt d permissions " . $file . " | ") {
		print $!;
		return 0;
	}
	...
	while (<FILE>) {
		my $output = $_;
		foreach (&PERMS) {
			my $search = PERM_SIGNATURE . $_;
			if ($output =~ m/$search/ ) {
		 		print "WARNING: " . $search . " found!\n";
			}
		}
	}
	close FILE;
...
$#ARGV == 0	or die "Usage: apkperm.pl <apk-file>\n";

my $fileName = $ARGV[0];
my $dirName = basename($fileName, ".apk");

print "APK Permission Check Tool 1.0, scanning " . $fileName . "\n";

checkManifest($fileName)
	or die "Could not check AndroidManifest.xml\n";
